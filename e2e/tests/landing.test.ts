...
test('Check header', async t => {
  await t
    .expect(landingPage.getText('headline'))
    .eql('Digital Leksehjelp');
});
...
test('Check QA description', async t => {
  await t
    ...
    .eql('Her kan du lete etter svar blant allerede stilte spørsmål, eller stille et nytt spørsmål hvis du ikke finner det du lurer på!');
});

//Test footer text value
test('Check footer', async t => {
  await t
    .expect(landingPage.getText('footer'))
    .eql('Følg oss på Facebook');
});
